...
ifeq "$(wildcard $(KERNEL_SRC) )" ""
    ...
    ifeq ($(TARGET_KERNEL_CONFIG),)
        ...
        ifneq ($(TARGET_FORCE_PREBUILT_KERNEL),)
            ifneq ($(filter RELEASE NIGHTLY SNAPSHOT EXPERIMENTAL,$(LUMINA_BUILDTYPE)),)
...
define build-image-kernel-modules-lumina
...
$(TARGET_PREBUILT_INT_KERNEL): $(KERNEL_CONFIG) $(DEPMOD) $(DTC) $(PAHOLE)
	...
	$(hide) if grep -q '=m' $(KERNEL_CONFIG); then \
			...
			($(call build-image-kernel-modules-lumina,$$modules,$(KERNEL_MODULES_OUT),$(KERNEL_MODULE_MOUNTPOINT)/,$(KERNEL_DEPMOD_STAGING_DIR),$(BOARD_VENDOR_KERNEL_MODULES_LOAD))); \
			$(if $(BOOT_KERNEL_MODULES),\
				...
				[ $$? -ne 0 ] && exit 1; \
				($(call build-image-kernel-modules-lumina,$$vendor_boot_modules,$(KERNEL_VENDOR_RAMDISK_MODULES_OUT),/,$(KERNEL_VENDOR_RAMDISK_DEPMOD_STAGING_DIR),$(BOARD_VENDOR_RAMDISK_KERNEL_MODULES_LOAD))); \
			) \
		fi
