function __print_lumina_functions_help() {
cat <<EOF
Additional Luminarica functions:
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the Luminarica model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch lumina_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/lumina-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.lumina.device | grep -q "$LUMINA_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $LUMINA_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.lumina.device | grep -q "$LUMINA_BUILD");
    then
...
function cmka() {
    if [ ! -z "$1" ]; then
        for i in "$@"; do
            case $i in
                celestia|otapackage|systemimage)
                    mka installclean
                    mka $i
                    ;;
                *)
...
function dopush()
{
    ...
    if (adb shell getprop ro.lumina.device | grep -q "$LUMINA_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $LUMINA_FIXUP_COMMON_OUT ]; then
