...
func init() {
	android.RegisterModuleType("lumina_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = luminaExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = luminaExpandVariables(ctx, dep_file)
	...
	cmd := luminaExpandVariables(ctx, String(g.properties.Cmd))
